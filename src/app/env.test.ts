...
import { ConfigError, decideBoot, readConfig, type AppConfig } from './env';
...
// La DECISIONE di boot — montare React o no — vive in decideBoot(), non nel
// DOM: la proviamo qui in ambiente node senza jsdom/happy-dom (dip vietate).
// main.tsx applica solo l'effetto sul DOM a partire da questa decisione (AC5).
describe('decideBoot', () => {
  it('source valida ⇒ kind: ok con l\'AppConfig atteso', () => {
    const decision = decideBoot({ ...validSource });

    expect(decision.kind).toBe('ok');
    // Narrowing sul discriminante: config presente solo nel ramo 'ok'.
    if (decision.kind !== 'ok') throw new Error('atteso kind: ok');
    expect(decision.config).toEqual<AppConfig>({
      supabaseUrl: 'https://xyzcompany.supabase.co',
      supabaseAnonKey: 'anon-key-abc123',
    });
  });

  it('VITE_SUPABASE_URL mancante ⇒ kind: config-error che nomina la variabile', () => {
    const decision = decideBoot({
      VITE_SUPABASE_ANON_KEY: validSource.VITE_SUPABASE_ANON_KEY,
    });

    expect(decision.kind).toBe('config-error');
    if (decision.kind !== 'config-error') throw new Error('atteso kind: config-error');
    expect(decision.error).toBeInstanceOf(ConfigError);
    expect(decision.error.message).toMatch(/VITE_SUPABASE_URL/);
    expect(decision.error.variables).toContain('VITE_SUPABASE_URL');
  });

  it('errore inatteso (non ConfigError) ⇒ rilanciato, non mascherato', () => {
    const hostile: Record<string, unknown> = {};
    Object.defineProperty(hostile, 'VITE_SUPABASE_URL', {
      get() {
        throw new TypeError('accesso ostile alla source');
      },
      enumerable: true,
    });

    expect(() => decideBoot(hostile)).toThrow(TypeError);
  });
});
