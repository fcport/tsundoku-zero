...
describe('seed committato — non deve divergere da content/lessons/ (anti-drift)', () => {
  ...
  /**
   * Le lezioni REALI, lette e parsate dal disco (ordine deterministico). La
   * discovery RICALCA quella della CLI `discover()` (ricorsiva, solo file, `.json`
   * case-insensitive, ordinata): la guardia anti-drift deve rispecchiare ciò che
   * il generatore emetterebbe anche se il contenuto un giorno si annidasse in
   * sottocartelle — altrimenti un file nidificato produrrebbe un falso divario.
   */
  function realLessons(): Lesson[] {
    const paths = readdirSync(contentDir, { withFileTypes: true, recursive: true })
      .filter((dirent) => dirent.isFile() && dirent.name.toLowerCase().endsWith('.json'))
      .map((dirent) => join(dirent.parentPath, dirent.name))
      .sort();
    return paths.map((path) => {
      const parsed = parseLesson(JSON.parse(readFileSync(path, 'utf8')));
      if (!parsed.ok) {
        throw new Error(`${path} non valido: impossibile parsare`);
      }
      return parsed.value;
    });
  }
