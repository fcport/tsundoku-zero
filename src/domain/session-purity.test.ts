...
// Sonda MECCANICA sul SORGENTE di `session.ts`, modellata su
// `schedule-purity.test.ts` (AD-1, purezza del dominio). `no-restricted-globals` in
...
// l'istante `now` deve ENTRARE dentro l'evento (`sessionReducer.length === 2`).

const __dirname = dirname(fileURLToPath(import.meta.url));
const sessionSource = resolve(__dirname, 'session.ts');
...
describe('purezza temporale di session.ts (AC3, AD-1)', () => {
  ...
  const raw = readFileSync(sessionSource, 'utf8');
  it('trova il sorgente di session.ts (anti-vacuità)', () => {
    expect(raw.length).toBeGreaterThan(0);
    expect(raw).toContain('export function sessionReducer');
  });
  ...
  it('now entra dentro l evento: sessionReducer.length === 2', () => {
    expect(sessionReducer.length).toBe(2);
  });
});
