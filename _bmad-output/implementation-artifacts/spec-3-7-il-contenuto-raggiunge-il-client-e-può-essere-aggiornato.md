...
- addressed_findings:
  - `[low]` `[patch]` La guardia anti-drift (`realLessons()` in `src/generate-content-seed.test.ts`) scopriva i file con `readdirSync` **non ricorsivo**, mentre la CLI `discover()` è **ricorsiva**: con contenuto piatto coincidono, ma appena una lezione fosse annidata in una sottocartella la guardia divergerebbe dal generatore (falso fallimento CI). Allineata la discovery del test a quella della CLI (ricorsiva, solo file, `.json` case-insensitive, ordinata per path). Verifica: `npm run lint`/`typecheck`/`test` (586 test)/`validate-content` tutti verdi.

## Auto Run Result

Status: done — review pass di follow-up su una storia già `done` (nessun difetto bloccante trovato).

### Sintesi del cambiamento (già a baseline `e57470e`, invariato in sostanza)
La storia porta il contenuto al client come **righe di Postgres popolate da migrazione** (OQ-8): due tabelle in sola lettura (`lesson`, `exercise`) con RLS abilitata e una sola policy `select` per gli autenticati, più un **seed idempotente** (`insert … on conflict (id) do update`) generato dai file `content/lessons/` riusando gli identificatori del dominio (`lessonId`, `deriveExerciseId`). Il titolo è bilingue (`title_en` obbligatorio, `title_it` nullable); `explanation_it` ricade su `null` (FR8.5). Applicazione solo su `main` via `migrate.yml`; validazione SQL **offline** (pg-query-emscripten), nessun Supabase locale.

### File toccati in questo pass di follow-up
- `src/generate-content-seed.test.ts` — la guardia anti-drift ora scopre le lezioni reali in modo **ricorsivo**, coerente con la CLI `discover()`.

### Ripartizione dei finding (questo pass)
- **Patch applicati: 1** (low) — allineamento discovery della guardia anti-drift.
- **Deferred: 0** — l'accumulo dei seed timestamp-ati senza prune resta l'unico item già registrato in frontmatter `deferred` (medium), non riaperto.
- **Reject: 20** (medium 2, low 18) — tra i principali, tutti verificati contro il codice reale:
  - Collisioni di `lessonId`/`deriveExerciseId`/`order` nel seed: già intercettate cross- e intra-file da `validateLessons` (`src/domain/content-validation.ts:80-184`), eseguita da `main()` con `process.exit(1)` **prima** del build → non raggiungibili da `buildSeedSql`.
  - `JSON.parse` in `main()`: preceduto dallo stesso `validateLessons` che rifiuta il JSON malformato (exit 1) → il secondo parse vede solo JSON già valido.
  - Escape backslash / `standard_conforming_strings`: `on` di default in Postgres moderno (Supabase) → i backslash sono letterali; nessun rischio.
  - RLS «provata» solo staticamente (struttura DDL, non deny/allow a runtime): forzato dal vincolo dell'intento stesso (nessun Supabase locale). La migrazione gira come proprietario dello schema (bypassa RLS), semantica standard.
  - Payload jsonb senza shape-check / test di round-trip col lettore: il consumatore (`ContentRepository`) è la storia 3.10, fuori scope.

### Verifica eseguita
- `npm run lint` → exit 0
- `npm run typecheck` (`tsc --noEmit`) → exit 0
- `npm test` → 59 file, **586 test passati**, exit 0 (inclusi `src/migrations.test.ts` e `src/generate-content-seed.test.ts` con la guardia anti-drift patchata)
- `npm run validate-content` → exit 0 (cancello FR2.6 verde)

### Rischi residui
- **Accumulo dei seed** (già `deferred`, medium): ogni rigenerazione scrive un nuovo `<timestamp>_seed_content.sql` senza rimuovere il precedente; lo stato finale del DB resta corretto (upsert idempotente, last-write-wins), ma i file di seed orfani si accumulano. La guardia «esiste esattamente un seed committato» costringe comunque a consolidare prima del commit. Serve una politica di prune/consolidamento (fuori scope 3.7).
- Nessun lettore a runtime delle tabelle esiste ancora in `src/data/` (previsto in 3.10): la promessa «il contenuto raggiunge il client» è, in questa storia, completata sul lato di **persistenza e distribuzione via migrazione**, non ancora sul lato di lettura applicativa.
